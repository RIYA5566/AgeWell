...
let globalApiAudioCtx = null;

function getApiUnlockedAudioContext() {
  if (!globalApiAudioCtx) {
    const AudioCtx = window.AudioContext || window.webkitAudioContext;
    if (AudioCtx) globalApiAudioCtx = new AudioCtx();
  }
  if (globalApiAudioCtx && globalApiAudioCtx.state === 'suspended') {
    globalApiAudioCtx.resume().catch(() => {});
  }
  return globalApiAudioCtx;
}

function playMessageNotificationSound() {
  try {
    const ctx = getApiUnlockedAudioContext();
    if (!ctx) return;
